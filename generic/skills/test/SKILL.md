...
- When the `code` skill implements a fix that the `debug` skill diagnosed, and the change needs confirming against the plan and the rest of the suite.
...
5. **Decide the outcome:**
   - **Failures or regressions** -> hand off to the `debug` skill to diagnose and report the root cause (it never writes the fix — the `code` skill implements what `debug` diagnoses).
...
- Remember the loop rules: the **`code`** skill never exits (always -> `debug` or `test`); the **`debug`** skill never writes — it diagnoses and routes to `code` (to implement) or `test` (to verify); the **`test`** skill is the sole loop-breaker.
