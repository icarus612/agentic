...
- **Workflow worktrees** — `CLAUDE_WORKFLOWS_DIR`, local default root `.workflows/`. Always gitignored; each worktree sits on a `<type>/<name>` branch (`type` ∈ `feature|bug|hotfix|docs|sync`, default `feature`) off the base branch. Runs that dispatch builders use the **parent/child scheme**: the run's parent worktree sits on `<type>/<name>-parent` off the base branch, and each builder lane's child worktree sits on `<type>/<name>-<lane-id>` cut off the parent branch via `workflow-setup.sh --parent` (modes without builders create the parent only). Children are ephemeral — merged back into the parent and removed lane by lane; only the parent branch is ever published.
...
Worktree isolation isn't only an orchestrator convention: `generic/hooks/worktree-reminder.sh` (a `SessionStart` hook) reminds ANY session — with or without the `dae` orchestrator driving it — to set one up via `workflow-setup.sh` before mutating files, and to route pushes/PR reviews through `push-pr`/`review-pr` rather than raw `git`/`gh` commands.
