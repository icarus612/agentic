...
- **`contracts/<child-id>.md`** — each builder's contract document, keyed by its child id (`c1`, `c2`, … in dispatch order — the same id that names its branch and worktree). This is its defined home: never inside the child worktree (where it would either die with the lane or leak into the product branch).
- **`reports/<child-id>-exit.md`** — each builder's exit report (envelope header + files touched + per-criterion evidence), written before the builder returns; its envelope points here. `validate-report.sh --kind exit` checks it; `verify-scope.sh` reads its file list.
