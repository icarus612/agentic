...
- **`contracts/<lane-id>.md`** — each builder's contract document. This is its defined home: never inside the child worktree (where it would either die with the lane or leak into the product branch).
- **`reports/<lane-id>-exit.md`** — each builder's exit report (envelope header + files touched + per-criterion evidence), written before the builder returns; its envelope points here. `validate-report.sh --kind exit` checks it; `verify-scope.sh` reads its file list.
