...
run_javascript_tests() {
    ...
    if ! command -v pnpm >/dev/null 2>&1; then
        if command -v npm >/dev/null 2>&1; then
            ...
            echo -e "${YELLOW}⚠️  No package manager found, skipping JS tests${NC}" >&2
            return 0
        fi
    fi
            ...
            case "$mode" in
                "focused")
                    if [[ -n "$test_file" ]]; then
                        ...
                        local test_output
                        if ! test_output=$(
                            $pkg_manager test -- "$test_file" 2>&1); then
                            failed=1
                            echo -e "${RED}❌ Focused tests failed for $base${NC}" >&2
                            echo -e "\n${RED}Failed test output:${NC}" >&2
                            format_test_output "$test_output" "javascript" >&2
...
main() {
    ...
    if [[ "$FILE_PATH" =~ \.go$ ]]; then
        ...
        # No tests for this file type or not a recognized project
        log_info "No applicable tests for this context"
        exit 0
    fi
