...
#
# CONFIGURATION
#   Requires ~/.config/.claude-ntfy/config.yaml with:
...
CONFIG_FILE="${CLAUDE_HOOKS_NTFY_CONFIG:-$HOME/.config/.claude-ntfy/config.yaml}"
...
case "$EVENT_TYPE" in
    "notification")
        ...
        TITLE="$CONTEXT"
        TAGS=".claude,notification"
        ;;
    
    "stop")
        TITLE="$CONTEXT"
        MESSAGE="Claude finished responding"
        TAGS=".claude,stop,checkmark"
        PRIORITY="low"
        ;;
