...
#
# SYNOPSIS
#   workflow-setup.sh [--name <name>] [--base <branch>]
#
# DESCRIPTION
...
#   gitignored and creates a worktree on branch dev/<name> off the base
#   branch. Prints machine-readable WORKTREE/BRANCH/BASE lines.
#
# EXIT CODES
#   0 - worktree created
...
while [ $# -gt 0 ]; do
  case "$1" in
    --name) name="${2:-}"; shift 2 ;;
    --base) base="${2:-}"; shift 2 ;;
    *) err "unknown argument: $1 (usage: workflow-setup.sh [--name <name>] [--base <branch>])" ;;
  esac
done
