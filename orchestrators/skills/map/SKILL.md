...
1. **Confirm scope and docs target.** For a monorepo, decide WITH the user whether to map the entire repo or one app and its dependencies. Set the target before exploring. Then resolve the docs target once via Bash: `~/.claude/hooks/resolve-config.sh CLAUDE_DOCS_DIR --default /docs` (global install path; use the project's `.claude/hooks/` copy if installed project-specific). Do NOT ask the user which branch to base the run on — that's resolved silently in step 2 via `CLAUDE_BASE_BRANCH` (per `artifact-locations`); only surface it if resolution genuinely fails. A filesystem path (the default `/docs`) means **local mode** (`document-local` is the documentation phase); an Atlassian wiki URL or the `confluence:<SPACE>[/<Parent Page>]` shorthand means **Confluence mode** (`document-confluence` publishes there instead — Confluence is the docs source of truth, no local `/docs` is maintained). In Confluence mode, confirm the space/parent page parsed from the value and the Google Drive folder for large-artifact offload with the user now — the document fork can't converse.
2. **Set up the workflow worktree.** Run `~/.claude/hooks/workflow-setup.sh --reuse --name map-repo` via Bash (global install path; use the project's `.claude/hooks/` copy if installed project-specific) — `map-repo` when bootstrapping docs from nothing, `remap-repo` when refreshing existing ones. The script resolves the base branch itself via `CLAUDE_BASE_BRANCH` (per `artifact-locations`) — only ask the user if it reports the base is unresolvable, then re-run with `--base <branch>` explicit. The branch is ALWAYS `feature/map-repo` or `feature/remap-repo`; never invent another name. `--reuse` picks up the branch left by a previous run and brings it up to date by merging the resolved base into it; if the script reports merge conflicts, stop and bring them to the user. Then `cd` into the printed WORKTREE path: ALL subsequent phases run inside it, and every path passed to forked skills must be inside it.
...
5. **Document.** Dispatch by the docs target from step 1 — both are `context: fork` skills invoked directly via the Skill tool:
   - **Local mode** → invoke **`document-local`**. Pass as args: the full structured map from the explore step, a statement that this is a **map-driven** run (no plan path, no build summary, no diff), and the changelog preference (commit, `docs/changelog`, both, or none — ask the user if unstated and it matters). The skill treats the map as ground truth for what to add, update, and delete under root `/docs`.
   - **Confluence mode** → invoke **`document-confluence`**. Pass as args: the parsed Confluence target (space, parent page), the Drive folder, the full structured map, a statement that this is a **map-driven** run (technical-only — no story file, no Jira keys unless the user supplied them), and the changelog preference. It publishes the technical map to Confluence pages. If it returns a needs-input report (missing target, dead connection), resolve that with the user and re-invoke — don't let it guess.
...
- Docs, plans, and base-branch locations come only from `CLAUDE_DOCS_DIR`/`CLAUDE_PROJECT_PLANS_DIR`/`CLAUDE_BASE_BRANCH` (resolved via `resolve-config.sh`, per `artifact-locations`) or their defaults (root `/docs`, `/project-plans/`, git heuristic) — never asked conversationally, never CLAUDE.md prose.
