...
| `--sh[ip]` | *(none)* | `chat` \| `publish` | per the resolved type row |
...
- **The bracket minimums are `--t`, `--exp`, `--rig`, `--ag`, `--sh`, `--pl`** — this is what `resolve-type.sh` actually accepts (its `FLAG_TABLE` and `resolve_flag()` are the authority). A token shorter than its flag's minimum is an **unknown flag**, never guessed at.
...
- **`--pipeline` is internal-only and never appears as a row in this table.** A type IS its pipeline, so `--pipeline` is **locked**: recognized only in order to be **rejected** with a plain error naming the type's actual pipeline, never treated as an unknown flag. `dae map --pipeline build` is a category error, not a customization.
- **Override tiers:** `--explore` and `--rigor` are **free** (cost/quality dials; an override can never produce an incoherent run — this is the escape hatch that keeps types from proliferating); `--against` and `--plan` are **constrained** (validated mechanically at parse time, before setup); `--pipeline` is **locked**. Types are the API; axes are internals.
...
| `analyze` | `report` | `report.md` | — | `docs/` | forbid | chat (`--ship publish` allowed) |
...
- **`map` vs `analyze`:** the same pipeline at different presets — `map` is fast (`explore: auto`, `rigor: low`), `analyze` is rigorous (`explore: deep`, `rigor: {explore: med}`). They differ ONLY in axis values, never in shape, which is why one file serves both.
...
**Non-zero exit = a constraint violation.** Present its plain-language ask to the user and STOP — before setup, before any worktree exists. The error set: `--against` arity against the row's `require|optional|forbid`, `--plan` placement (`pipeline: build` types only), locked `--pipeline`, unknown flags, unknown type, malformed table.
...
| Confluence docs target | `confluence-mode.md` + `document-confluence` records |
