...
- **Every subphase detail block declares its test oracle** — `new contract tests` | `existing suite` | `equivalence check` | `existing implementation` (the shipped implementation's observed behavior is the truth source: tests are derived from it and expressed as contracts, contract-testers only, no coder). The builder's packet machine reads this field to size its contract-tester fan-out and stays type-blind; your module says which oracle its type defaults to.
