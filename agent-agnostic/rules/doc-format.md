...
# Doc format
All documentation lives in the docs root (see `artifact-locations`) — never scattered through the source tree. The mirror/symlink structure below applies unconditionally; there is no other structure.

Docs live in the repo under the resolved `CLAUDE_DOCS_DIR` (see `artifact-locations` for the variable). When `CLAUDE_DOCS_PUBLISH` is set, the docs tree is additionally mirrored to that target by a CI job on merge — never by an agent, and never as a stage of any run. `document-confluence` exists only as a manual/recovery sync for when CI has not run or has failed. Publication is one-way, repo → target: Confluence is read-only for humans, and an edit made there is not a source change — it is overwritten by the next sync. Reading docs always means reading the local tree, in every configuration. Why: `review-pr` judges the branch diff, and docs that live only in Confluence would not be in it.
...
This rule is format and placement only — what to document and when is the documenting skill's job; how a published copy is produced is CI's job.
