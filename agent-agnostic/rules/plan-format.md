...
- **Where a plan lives, and its lifecycle.** The plans dir root holds NO loose plan files: only the two standing dirs `proposals/` and `completed/`, and one directory per plan.
  - **Proposal.** An unapproved plan is a single file `<plans-dir>/proposals/<slug>-MM-DD-YY.md`. Records produced before approval — the gate's `<slug>-MM-DD-YY.plan-review.md`, a `confluence-mode.md` run's `<slug>-MM-DD-YY.story.md` — sit beside it as siblings until it is promoted.
...
- **Required sections.**
  ...
  - **Goal & scope** — what's in, and explicitly what's out. Also carries a mandatory **ask-of-record pointer**: a plan is the second hop of a two-hop transformation (**ask → plan → code**), and until now only the second hop was ever audited — every gate judged the implementation against the plan; nothing compared the plan against the ask. The pointer takes one of two forms, never left implicit:
    - **A path** to the run's durable ask artifact — the ask captured *before the planner was spawned*, so the brief is a file rather than conversation context (this run's own `<run-dir>/the-ask.md` is the worked example). A `confluence-mode.md` run's `story.md` already opens with *"Original Ask (verbatim)"* and satisfies the pointer on its own — no second artifact is required, though `story.md` is not mandated outside such a run.
    - **An explicit statement that no durable ask exists**, written down, never left blank. A run whose ask is only conversation context has nothing to diff against, so the gate's ask-vs-plan check cannot run — and everyone downstream must know that rather than assume it passed.
