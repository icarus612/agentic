...
3. **Confirm, then post.** Posting to a PR is outward-facing: it rides on the explicit user approval your args assert (and the permission prompt). Post as a single PR comment via `gh pr comment <pr> --body-file <rendered>`, or the GitHub MCP **only if `gh` is not installed**. A denial is not unavailability: if `gh` exists but the call is denied by a permission rule or blocked by the auto-mode classifier, STOP and report the blocked command to the caller — never reroute the same post through the MCP, `gh api`, or `curl`. The caller's approval authorizes the **outcome**, never a **route** the harness refused. If a previous comment from an earlier round exists, post the new round as a new comment — the PR thread is the history; never edit or delete earlier ones.
