...
**1. Setup** (shared — router). Branch type `bug/`. Additionally capture, conversationally, BEFORE setup completes: the **issue** precisely (freeform description/stack trace VERBATIM, a plan path, or a Jira key — fetch the ticket via the Atlassian MCP yourself; at least one source required, never invented) and the **suspect work** (`--against`/`-a`, an array of anchors — branches, ranges, SHAs, PRs, tickets, plan paths; repeatable flag and comma-separated both accepted, order preserved, more than one anchor narrows the hunt; or ask for a branch/PR/range/merge locator — never guess it from the issue text). Resolution is `resolve-anchor.sh`'s (install `~/.claude/hooks/`, or the project's `.claude/hooks/` copy), all-or-nothing — one unresolvable anchor fails the whole parse; the resolved `anchors:` block is written to the plan front-matter and the progress log so a resumed or amended run reasons about exactly the same set.

**2. Plan ‖ init.** Spawn the **`planner`** agent with type `diagnose` (module `plan-diagnosis`), the issue verbatim, the resolved suspect anchor array, the parent worktree path, and the `--explore` override if any (this workflow's default is DEEP). The planner diffs, investigates — fanning out its own investigation subagents — and writes the ranked diagnosis report to `proposals/`; its envelope points at it. `init-workspace` runs in parallel, joined before the gate.
