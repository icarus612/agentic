...
push_msg='Before running git push or opening/commenting on/reviewing a pull request in ANY session — not just one driven by an orchestrator — prefer the push-pr/review-pr skills over raw git/gh commands: they enforce push-policy (`dev` is the universal integration branch — always branch from it and PR into it, always squash-merge, never commit directly on it, never force-push) and the staged publish flow: `push-pr --stage open-draft` opens the PR as a draft right after plan approval (conversational confirmation held), `push-pr --stage update` pushes the parent branch again after every lane merge-back and record commit with no confirmation asked, and `push-pr --stage finalize` pushes the last stragglers and flips the PR from draft to ready (conversational confirmation held again).'
