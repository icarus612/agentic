...
- **Approved:** the plan is verified; recommend approval. Once the user signs off at the caller's gate, the caller switches to auto mode and hands off to the code/debug/test loop, starting with the `code` skill.
