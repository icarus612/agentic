---
name: map
description: Entry-point orchestrator for documentation-only runs on a dedicated feature/{re,}map-repo branch — a deep explore, init-workspace, a map-driven document pass, then push-branch — to bootstrap or refresh root /docs when there is no code change to record. Invoke as /map.
type: workflow
domain: dev
...
6. **Finish (& tear down).** Invoke the **`push-branch`** skill directly via the Skill tool — also a `context: fork` skill. Pass as args the worktree path, the branch name, and confirmation the run is completely done. It commits any straggler docs, pushes the workflow branch (always asking the user first — a declined push is a valid reported outcome, never worked around), and removes the worktree. The `map` orchestrator is NOT done until `push-branch` reports.
7. **Report.** Surface the document skill's summary (docs paths written, symlinks created or repaired, changelog outcome) and `push-branch`'s outcome (pushed or declined, worktree removed) together with anything the `explore` skill flagged as unverified.
...
- Never push from the `map` orchestrator itself — publishing happens ONLY through `push-branch`, which always asks the user first, never force-pushes, and never pushes main. The document skill may `git add` + `git commit` a changelog inside the worktree.
