...
When documenting a **change**, don't start cold: documentation must reflect what was actually built and reviewed, not a plan — if the `review-code` gate hasn't happened, say so and defer. The exception is a **map-driven** run, where a deep explore map arrives via args and writing or refreshing docs from it is the whole point.
...
2. **Confirm what changed, then mark the plan.** Check the plan in `/project-plans/`, the `review-code` outcome, and the actual diff (`git diff`, `git status`). Document the real, final state of the code, never an aspiration. If something in the plan was dropped or changed during the build loop, document what shipped. Then update the plan itself: in the high-level syllabus check off (`- [x]`) the phases now complete and annotate abandoned ones as `- [dropped]`, and annotate any task that was dropped or changed in the phase sections — the syllabus must reflect reality just like the docs do. In a map-driven run there is no plan or diff — the explore map is your ground truth; reconcile the existing `/docs` tree against it and add, update, or delete accordingly.
