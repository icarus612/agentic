...
2. **Gather the facts.** Read the plan (and mark its syllabus like `document-local` would: check off `- [x]` completed phases, annotate `- [dropped]` ones), the story file, and the actual diff (`git diff`, `git status`). Document the real, final state of the code, never an aspiration. In a map-driven run the explore map is your ground truth instead.
