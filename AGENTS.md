...
- **Skills** — on-demand procedures, `skills/<name>/SKILL.md` (+ optional
  sibling detail files and a `scripts/` dir, loaded/run only when needed).
  Selected by `description` or an explicit `/name`, **never** by folder.
  - *Orchestrators* are invoked by the user (`/dae`, `/orchestrate`).
  - *Universal forks and gates* are invoked **by name by an orchestrator or
    worker**, so each description states the pipeline it belongs to — that
    guard is what stops single-word skills (`explore`) auto-firing on a stray
    keyword.
