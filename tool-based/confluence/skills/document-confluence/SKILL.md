...
- **Change-driven**: the plan dir's `plan.md` (`<plans-dir>/<slug>-MM-DD-YY/plan.md`), the story file path (its `story.md`, same dir — original ask verbatim, narrative, acceptance criteria, constraints, stakeholders), and the build/`review-code` summary.
...
- **Reconciliation-driven** (from `sync-status`): the plan dir's `plan.md` (if one exists), its `sync-report.md` (same dir), and the resolved Jira issue key(s) — resolved by `sync-status` from either a user-supplied key or the plan dir's `story.md`, NEVER guessed by you. The **story file is optional** in this shape (the work may have shipped outside a tracked `dev` run, so none may exist) — if absent, publish without the story/narrative sections rather than inventing one, same as a map-driven run does. The Jira issue key(s) remain MANDATORY regardless — if `sync-status` didn't resolve one and Confluence mode is active, that's a needs-input condition, not something you infer.
